#!/usr/bin/env python3
"""
Script para testar o agente intervention-planner do OpenCode.
Executa uma chamada ao agente e valida o comportamento esperado.
"""
...
def run_opencode_command(prompt):
    ...
    cmd = ['opencode', '--agent', 'intervention-planner', 'run', prompt]
    ...
    try:
        ...
        return result.returncode, "", ""  # Como não capturou, retorna vazio
...
def main():
    prompt = 'I need to fix a bug in my code without changing anything.'

    print(f"Executando teste do intervention-planner com prompt: {prompt}")
    print("=" * 60)

    # Executa o comando
    print("1. Executando comando opencode...")
    returncode, stdout, stderr = run_opencode_command(prompt)

    print(f"Return code: {returncode}")
    # Não imprime stdout/stderr pois não foram capturados
    ...
    # Validação final
    success = manifesto_created  # Pelo menos o manifesto deve ser criado
    ...
    if success:
        print("✓ Teste PASSOU: Manifesto criado com sucesso")
        print("O agente intervention-planner funcionou corretamente:")
        print("  - Coletou contexto do projeto")
        print("  - Gerou plano de intervenção")
        print("  - Criou manifesto estruturado")
        return 0
    else:
        print("✗ Teste FALHOU: Manifesto não foi criado")
        print("Possíveis causas:")
        print("  - Agente não conseguiu executar a ferramenta create-intervention")
        print("  - Erro na configuração das permissões")
        print("  - Problema com o comando opencode")
        return 1
