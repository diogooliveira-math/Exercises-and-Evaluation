...
def main():
    ...
    if exercise_path_inputs:
        ...
        for p in exercise_path_inputs:
            p = p.strip()
            # If path contains ExerciseDatabase, strip leading parts
            if 'ExerciseDatabase' in p:
                # find substring after ExerciseDatabase
                idx = p.find('ExerciseDatabase')
                rel = p[idx + len('ExerciseDatabase'):].lstrip('/\\')
                ex_paths.append(EXERCISE_DB / Path(rel))
            else:
                cand = Path(p)
                if not cand.is_absolute():
                    # try relative to repo root
                    cand = PROJECT_ROOT / p
                ex_paths.append(cand)
        ...
        try:
            ...
            lines.append("% Generated by generate_test_from_ips.py")
            lines.append("")
            lines.append("% Inline support: input-path, setup-counter and include-exercise")
            # Inline the content of exercises.d files to avoid \input issues
            try:
                input_path_file = qa2_exercises_d / 'input-path'
                if input_path_file.exists():
                    lines.append('% --- begin input-path ---')
                    lines.extend(input_path_file.read_text(encoding='utf-8').splitlines())
                    lines.append('% --- end input-path ---')
            except Exception:
                pass

            for support_file in ['setup-counter.tex', 'include-exercise.tex']:
                try:
                    fpath = qa2_exercises_d / support_file
                    if fpath.exists():
                        lines.append(f'% --- begin {support_file} ---')
                        lines.extend(fpath.read_text(encoding='utf-8').splitlines())
                        lines.append(f'% --- end {support_file} ---')
                except Exception:
                    pass
